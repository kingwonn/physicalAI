...
| GO-1 | AgiBot (CN) | 2025-03 | 2B VLM + planner + expert | InternVL2.5-2B | ViLLA: latent planner + diffusion action expert | open (CC BY-NC-SA 4.0) |
...
| GR-3 | ByteDance Seed (CN) | 2025-07 | 4B | Qwen2.5-VL-3B | flow-matching DiT head | closed |
...
- Chinese labs shipped a dense wave of VLAs in 2025–26, skewing heavily toward open weights as a distribution strategy: ByteDance Seed **GR-3** and AgiBot **GO-1** (both detailed below); Unitree's open-sourced **UnifoLM-VLA-0** (weights + code released 2026-01-29) driving G1 household tasks; X Square Robot's **WALL-OSS** open 4B model (2025-09), with the WALL-OSS-0.5 technical report (2026) adding single-stage training on a 90M-sample multimodal corpus, and **WALL-B** (2026-04), a unified foundation model spanning perception, language, action, and physical prediction — the company closed four consecutive rounds ending in a Series C at a >$2.8B (RMB 20B) valuation, claiming lead-round backing at different stages from Alibaba, ByteDance, Meituan, and Xiaomi (company release, as of 2026-06).
- **GR-3** (ByteDance Seed; technical report 2025-07-21, arXiv:2507.15493): 4B-param VLA — Qwen2.5-VL-3B-Instruct backbone + flow-matching diffusion-transformer (DiT) action head — trained by co-training on web-scale vision-language data, fine-tuning on human trajectories collected via VR devices, and imitation learning on robot teleop; controls **ByteMini**, ByteDance's bimanual mobile robot. Reports beating π0 across its three eval suites (e.g. 97.5% success on long-horizon table bussing instruction-following; 86.7% average progress on dexterous cloth manipulation) (self-reported). No public weights as of 2026-07. Not to be confused with Fourier's GR-3, a care-focused humanoid *hardware* platform (see [Landscape: China](landscape-china.md)).
- **GO-1** (AgiBot, 2025-03; arXiv:2503.06669): **ViLLA** architecture (vision-language-latent-action) — InternVL2.5-2B VLM + 24-layer latent planner predicting discrete latent action tokens + diffusion action expert for continuous control; pretrained on **AgiBot World** (>1M trajectories, 2,976 h, 217 tasks, collected on 100 dual-arm humanoid robots). Paper reports +30% average over Open X-Embodiment pretraining and >60% success on complex real-world tasks (+32% vs RDT). Dataset, code, and weights (GO-1 and the lighter GO-1-Air, which drops the latent planner) are open on Hugging Face under CC BY-NC-SA 4.0; IROS 2025 Best Paper Award finalist.
...
- The widely repeated "100+ Chinese VLAs" figure traces to ResearchInChina's *VLA Large Model Applications in Automotive and Robotics Research Report, 2025* (published 2025-09). Its actual claim is narrower than the paraphrase: "more than 100 VLA models **and related data sets** in the robotics field" — a field-wide tally that mixes models with datasets and is not restricted to Chinese labs (methodology unpublished; market-research estimate). The same report separately counts 40+ large-model frameworks/solutions and surveys VLA-style driving stacks at Chinese automakers (XPeng, Li Auto, Chery, Geely, Xiaomi Auto). Details in [Landscape: China](landscape-china.md).
...
- https://arxiv.org/abs/2507.15493 — ByteDance Seed GR-3 technical report (2025-07-21): 4B params, Qwen2.5-VL-3B backbone + flow-matching DiT head, VR-collected human trajectories, ByteMini robot, π0 comparisons
- https://arxiv.org/abs/2503.06669 — AgiBot World Colosseo paper: >1M trajectories / 2,976 h / 217 tasks on 100 robots; GO-1 ViLLA architecture (InternVL2.5-2B + latent planner + diffusion action expert); +30% over OXE pretraining
- https://huggingface.co/agibot-world/GO-1 — GO-1 open weights (CC BY-NC-SA 4.0); GO-1-Air lightweight variant also released
- https://www.globenewswire.com/news-release/2025/09/11/3148371/28124/en/China-VLA-Large-Model-Applications-in-Automotive-and-Robotics-Research-Report-2025-Robots-on-the-Rise-Over-100-VLA-Models-Poised-to-Transform-Industries.html — ResearchInChina report (2025-09-11): origin of the "more than 100 VLA models and related data sets in the robotics field" count
